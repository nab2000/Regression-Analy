---
title: "Reg Analysis Course Project"
author: "NDR"
...
##Analysis
To start the analysis, first the data and our necessary libraries were loaded in:
```{r}
library(GGally)
library(ggplot2)
data("mtcars")
```
To examine what variables might have an impact on mpg, a pairs plot was constructed (Appendix Figure 1). Based on a review of this pairs plot, there appears to be a relationship between mpg and all the other variables, except the number of forward gears and the number of carburetors. 
...
summary(fit2)$coef
```
When weight is added into the linear model, the significance of transmission type disseapears as can be seen from the summary of fit2 coefficients. When we add additional vraibles weight continues to be the only variable whose coefficient is significant (except in model 3 where hp is also significant)

To evaluate these models, we used analysis of variance.
```{r}
anova(fit1, fit2, fit3, fit4, fit5, fit6, fit7, fit8)
```
The analysis of the different models shows that model 2 (transmission and weight) and 3 (transmission, weight, and hp) are significantly different from model 1. This suggests that weight and horsepower are important variables impacting mpg. To evaluate if weight is even important, a similar analysis was done with 3 new models, adding in transmission type as the last variable. 
```{r}
